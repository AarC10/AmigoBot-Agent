...
if [ "$(id -u)" = '0' ]; then
  ...
  # Only chown build artifacts if they are currently root-owned
  ...
  # If workspace root itself is owned by root change ownership to host user
  WS_OWNER_UID=$(stat -c %u "${WORKSPACE}" 2>/dev/null || true)
  if [ "${WS_OWNER_UID}" = "0" ]; then
    chown "${HOST_UID}:${HOST_GID}" "${WORKSPACE}" 2>/dev/null || true
  fi
else
  # Running as non-root inside container - try to create dirs if parent is writable
...
# -------------------------------
# Load ROS environment + workspace
# -------------------------------
ROS_SETUP="/opt/ros/${ROS_DISTRO:-noetic}/setup.bash"
if [ -f "${ROS_SETUP}" ]; then
...
WS_INSTALL_SETUP="${WORKSPACE}/install/setup.bash"
WS_DEVEL_SETUP="${WORKSPACE}/devel/setup.bash"
